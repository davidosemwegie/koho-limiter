...
const velocityLimitFileScanner = (input: string) => {
  const data_store = new Map()

  fs.readFile(input, "utf8", (err, data: any) => {
    if (err) {
    ...
    new_array.forEach((line: string, index: number) => {
      ...
      //Check if current values are not 0

      if (
        data_store[date][customer_id]["daily_deposit_balance"] === undefined &&
        data_store[date][customer_id]["number_of_deposits"] === undefined &&
        data_store[date][customer_id]["weekly_deposit_balance"] == undefined
      ) {
...
export default velocityLimitFileScanner
